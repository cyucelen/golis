...
get_build_command = $(strip $(foreach mode,$(1)_MODE, \
    $(if $(strip $(DOCKERIZE)),\
      ...
      ,\
      $(MAKE) $(if $(strip $($(mode))),$(mode)=$($(mode)),) -C .)))
...
.PHONY: $(foreach i,$(DO_IMPLS),$(foreach s,$(STEPS),$(call STEP_TO_PROG,$(s))))
$(foreach i,$(DO_IMPLS),$(foreach s,$(STEPS),$(call STEP_TO_PROG,$(s)))):
	$(foreach impl,$(word 2,$(subst /, ,$(@))),\
	  $(if $(DOCKERIZE), \
	    $(call get_build_command,$(impl)) $(patsubst ./%,%,$(@)), \
	    $(call get_build_command,$(impl)) $(subst ./,,$(@))))
	...
	@$(foreach impl,$(word 2,$(subst ^, ,$(@))),\
	  $(foreach step,$(word 3,$(subst ^, ,$(@))),\
	    ...
	    $(foreach test,$(patsubst impls/%,%,$(call STEP_TEST_FILES,$(impl),$(step))),\
	      ...
	      echo 'Running: $(call get_runtest_cmd,$(impl),$(step)) ./$(test) -- ./run' && \
	      $(call get_runtest_cmd,$(impl),$(step)) ./$(test) -- ./run && \
	      $(if $(filter tests/$(argv_STEP)$(EXTENSION),$(test)),\
	        echo '----------------------------------------------' && \
	        echo 'Testing ARGV of $@; step file: $+' && \
	        echo 'Running: $(call get_argvtest_cmd,$(impl),$(step)) ./run ' && \
	        $(call get_argvtest_cmd,$(impl),$(step)) ./run  && ,\
		true && ))\
	    true))
...
$(DOCKER_BUILD):
	...
	$(foreach impl,$(word 2,$(subst ^, ,$(@))),\
	  ...
	  cd . && docker build -t $(call impl_to_image,$(impl)) .)
...
$(IMPL_STATS):
	@$(foreach impl,$(word 2,$(subst ^, ,$(@))),\
	  echo "Stats for $(impl):"; \
	  $(LOCCOUNT) -x "[sS]tep[0-9]_.*|[.]md$$|tests|examples|Makefile|package.json|tsconfig.json|Cargo.toml|project.clj|node_modules|getline.cs|terminal.cs|elm-stuff|objpascal/regexpr|rdyncall|swift/templates" .)
